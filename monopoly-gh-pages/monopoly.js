function Game() {
	...
	this.next = function() {
		if (!p.human && p.money < 0) {
			...
			if (document.getElementById("question-answer").value == s.answer || s.question == "" || questionAttempts == 3) {
				p.money += 60-(questionAttempts*20);
				updateMoney();
				play();
			} else {
				questionAttempts += 1;
				addAlert("That's the wrong answer. Remember " + s.explanation + " You have " + (3-questionAttempts) + " more tries.");
			}
		} else {
			roll();
		}
...
function showdeed(property) {
	...
	if (sq.mortgage) {
		...
		if (sq.groupNumber >= 3) {
			...
			document.getElementById("deed-question").textContent = sq.question;
...
function land(increasedRent) {
	...
	questionAttempts = 0;
	if (s.question != "") {
		addAlert("What is " + s.question);
	}
	document.getElementById("question-text").innerHTML = s.question;
	...
	// Go to Detention. Go directly to Jail. Do not pass GO. Do not collect $200.
	if (p.position === 30) {
		updateMoney();
		updatePosition();

		if (p.human) {
			popup("<div>Go to detention. Go directly to Jail. Do not pass GO. Do not collect $200.</div>", goToJail);
		} else {
			gotojail();
		}
